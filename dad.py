...
CAPABILITIES = {
    "services": ["LLM/GenAI", "RPA", "GIS/Digital Twin", "Data Platform", "Cyber/Zero Trust", "Edge GPU/5G", "Digital Health AI"],
    "frameworks": {"G-Cloud 14": True, "DOS 7": True, "NHS SBS": True, "RM6263 Cyber": False, "JOSCAR": False, "DFOCVC": True},
    "certifications": {"ISO 27001": True, "Cyber Essentials Plus": True, "ISO 13485": False, "DCB0129": True, "DCB0160": True, "NPPV3": False, "SC": False, "DV": False},
...
We maintain an ISMS aligned to ISO 27001 with documented SoA, risk treatment, and PDCA improvement. Controls cover access, crypto, ops, supplier and incident mgmt.
"""
...
**Portal:** {t['portal']}  
**Deadline:** {t['deadline']}  
**Estimated Value:** £{int(t['est_value_gbp']):,}
...
- Requirements coverage: {t['requirements']}
- Standards & compliance: {t['framework_required']}
...
- Build & Configuration: £{max(60_000, int(0.05 * float(t['est_value_gbp']))):,}
- Run (12 months): £{max(120_000, int(0.15 * float(t['est_value_gbp']))):,}
...
def build_submission_zip(t, proposal_md):
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        reqs = parse_req_list(t["requirements"])
        comp_csv = "Requirement,Response\n" + "\n".join([f"{r},Covered" for r in reqs]) + ("\n" if reqs else "")
        z.writestr(f"{t['id']}/compliance_matrix.csv", comp_csv.encode("utf-8"))
        pricing_rows = [
            ("Discovery", 45000),
            ("Build", max(60000, int(0.05 * float(t["est_value_gbp"])))),
            ("Run-12m", max(120000, int(0.15 * float(t["est_value_gbp"]))))
        ]
...
if "history" not in st.session_state: st.session_state.history = pd.DataFrame(columns=["id","title","buyer","score","result","notes","date"])

tenders = synth_tenders()
tabs = st.tabs(["1) Crawl","2) Classify","3) Score","4) Auto-Propose","5) Submit","6) Learn"])

with tabs[0]:
    st.subheader("Tender Feed (Synthetic)")
...
with tabs[5]:
    ...
    st.session_state.weights = {
        "fit_weight": fit_w/total, "value_weight": value_w/total, "deadline_weight": deadline_w/total,
        "compliance_weight": comp_w/total, "capacity_weight": cap_w/total
    }
...
**Data Sources**
- UK: Find a Tender, Contracts Finder, Atamis/Jaggaer/Bravo (CCS/NHS)
- EU: Funding & Tenders (Horizon Europe)
- Local Gov: Proactis, In-tend
